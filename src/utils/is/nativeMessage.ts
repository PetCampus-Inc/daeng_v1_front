...
type ResponseDataSchemaType = {
  [K in NativeMessageType<"Response">]: z.ZodType<NativeMessageData<"Response", K>>;
};
...
/**
 * 메세지가 `NativeMessageResponse` 타입인지 확인합니다.
...
export function isNativeMessageResponse<T extends NativeMessageType<"Response">>(
  ...
  const { type, data } = message as NativeMessageResponse<T>;
